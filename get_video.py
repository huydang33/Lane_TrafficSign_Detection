...
def run_video(detection_model, lane_model, frames_folder, output_video_path, device, threshold=0.5):
    ...
    fps = 30  # hoặc lấy từ video metadata nếu có
    ...
    for frame_path in frame_paths:
        ...
        with torch.no_grad():
            # ----- Lane prediction -----
            lane_out = lane_model(lane_input)  # Output shape: [1, 201, 18, 4]
            lane_out = lane_out.squeeze(0)     # [201, 18, 4]
            lane_class = torch.argmax(lane_out, dim=0)  # [18, 4]

            # Convert to binary mask
            binary_mask = (lane_class > 0).cpu().numpy().astype(np.uint8) * 255
            lane_mask_resized = cv2.resize(binary_mask, (w, h), interpolation=cv2.INTER_NEAREST)

            # Overlay lane mask
            mask_color = np.zeros((h, w, 3), dtype=np.uint8)
            mask_color[lane_mask_resized == 255] = [0, 0, 255]
